...
`answer_as_json()` may be especially powerful when your LLM provider has native support for
returning JSON objects (e.g., Ollama, OpenAI). 
Text-based handling is however always possible, also for providers which do not 
natively support such functions. This means that you can always switch between
