...
#> Error in `httr2::req_perform()` at tidyprompt/R/llm_provider.R:265:5:
#> ! Failed to perform HTTP request.
#> Caused by error in `curl::curl_fetch_memory()`:
#> ! Failed to connect to localhost port 11434 after 2250 ms: Couldn't connect to server
```
...
#> ! Failed to perform HTTP request.
#> Caused by error in `curl::curl_fetch_memory()`:
#> ! Failed to connect to localhost port 11434 after 2269 ms: Couldn't connect to server
```
...
#> ! Failed to perform HTTP request.
#> Caused by error in `curl::curl_fetch_memory()`:
#> ! Failed to connect to localhost port 11434 after 2259 ms: Couldn't connect to server
```
...
#> ! Failed to perform HTTP request.
#> Caused by error in `curl::curl_fetch_memory()`:
#> ! Failed to connect to localhost port 11434 after 2242 ms: Couldn't connect to server
```
...
#> ! Failed to perform HTTP request.
#> Caused by error in `curl::curl_fetch_memory()`:
#> ! Failed to connect to localhost port 11434 after 2248 ms: Couldn't connect to server
```
...
``` r
  ...
  print(utils::capture.output(print(temperature_in_location)))
#>  [1] "function (location = c(\"Amsterdam\", \"Utrecht\", \"Enschede\"), unit = c(\"Celcius\", "
#>  [2] "    \"Fahrenheit\")) "                                                                   
#>  [3] "{"                                                                                       
#>  [4] "    location <- match.arg(location)"                                                     
#>  [5] "    unit <- match.arg(unit)"                                                             
#>  [6] "    temperature_celcius <- switch(location, Amsterdam = 32.55, "                         
#>  [7] "        Utrecht = 19.8, Enschede = 22.7)"                                                
#>  [8] "    if (unit == \"Celcius\") {"                                                          
#>  [9] "        return(temperature_celcius)"                                                     
#> [10] "    }"                                                                                   
#> [11] "    else {"                                                                              
#> [12] "        return(temperature_celcius * 9/5 + 32)"                                          
#> [13] "    }"                                                                                   
#> [14] "}"                                                                                       
#> [15] "<environment: 0x000002455e5e8ab0>"
...
`add_tools()` can also be used to give the LLM access to pre-existing functions from packages.
The documentation will then be extracted from the available help files.
...
````
#> To get the list of files in your current directory, I will call the `list.files` function with the required arguments.
#> 
...
#>   "function": "list.files",
#>   "arguments": {
#>     "path": "." // '.' corresponds to the working directory
#>   }
#> }
#> ```
#> 
...
#> --- Receiving response from LLM provider: ---
#> It seems that `list.files` has returned a list of files in the current directory.
#> 
#> The output shows three files:
#> 
#> 1. `getting_started.Rmd`
#> 2. `getting_started.Rmd.orig`
#> 3. `precompile_vignettes.R`
#> 
#> These are the files present in the current directory.
#> 
#> Since this is the final result, I will provide it as my response.
#> 
#> **Final Response:**
#> 
...
#> [1] "It seems that `list.files` has returned a list of files in the current directory.\n\nThe output shows three files:\n\n1. `getting_started.Rmd`\n2. `getting_started.Rmd.orig`\n3. `precompile_vignettes.R`\n\nThese are the files present in the current directory.\n\nSince this is the final result, I will provide it as my response.\n\n**Final Response:**\n\nFiles in your current directory:\n\n* `getting_started.Rmd`\n* `getting_started.Rmd.orig`\n* `precompile vignettes.R`"
```
