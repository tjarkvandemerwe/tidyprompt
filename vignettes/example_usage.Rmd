...
Both extraction and validation functions can return feedback to the LLM, using the
`llm_feedback` function. When an extraction or validation function returns this, a
...
by the extraction functions) will be returned. (The `send_prompt` function is responsible
...
to break, forwarding to the return statement of `send_prompt`. See the `quit_if` function
