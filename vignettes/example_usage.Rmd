...
tidyprompt can be used with any LLM provider. At the moment, tidyprompts includes
pre-built functions to link with Ollama and the OpenAI API. With the create_llm_provider function,
you can write a hook for any other LLM provider that is capable of completing a chat.
...
A simple string serves as the base for a prompt. By adding prompt wrappers, 
you can influence various aspects of how the LLM handles the prompt, while
verifying that the output is correct (including retries with feedback if it is not).
...
the LLM can retry answering the prompt in the correct format.
...
improve the LLM's performance for certain tasks. 

For instance, set_mode_chainofthought will add chain of thought reasoning mode to the LLM.
This wraps the base prompt within a request for the LLM to reason step by step, asking
it to provide the final answer within 'FINISH[<final answer here>]'. An extractor
...
This enables the LLM to retrieve additional information in order to complete a task.
...
Under the hood, prompts are just lists of a base prompt and a series of prompt wrappers.

You can thus create a function which takes a prompt and appends a new prompt wrapper to it.

Take a look at the source code for add_text:
...
More complex may also add extractor functions and validation functions. 

They key difference between an extractor and validation function is that an 
extraction function alters the LLM's response and passes on the altered response
to any possible next extraction and/or validation functions, and eventually to the
return statement of send_prompt. A validation function, on the other hand, only
checks if the LLM's response passes a check. Both extractor functions and validation
functions can send feedback to the LLM.

For more information, on what you can do with prompt wrappers, see the documentation
of the prompt_wrap class creator: create_prompt_wrap. For examples, see, for instance
the source code of add_text, answer_as_integer, set_mode_chainofthought, and add_tools.
