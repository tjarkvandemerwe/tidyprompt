...
ollama$set_parameters(list(model = "llama3.1:8b"))
...
```{r tool_example_not_run, eval=FALSE}
  ...
  "Hi, what is the weather temperature in Enschede?" |>
    ...
    send_prompt(ollama, verbose = TRUE)
```
```{r fake_tool_result, echo=FALSE, warning=FALSE}
# Temporarily printing a fake tool result here, since the real thing 
#   fails when building the vignette via Github Actions (due to unknown reasons)
...
```{r wrap_extract_validate, echo=TRUE, eval=FALSE}
