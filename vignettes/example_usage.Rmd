...
```{r fake_llm_provider, include = FALSE}

# Setting up a fake LLM provider for environments in which a real LLM
...
```{r answer_as_integer1, cache=TRUE}
  "What is 2 + 2?" |>
    answer_as_integer() |>
...
```{r answer_as_integer2, cache=TRUE}
  "What is 2 + 2?" |>
...
Prompt wrappers may also be used to add a reasoning mode to the LLM. It is hypothesized that this could
...
```{r wrapper, echo=TRUE, eval=FALSE}
add_text <- function(
    prompt,
...
More complex prompt wrappers may also add extraction and validation functions. Take a look
at the source code for function `answer_as_integer`:

```{r wrapper_extract_validate, echo=TRUE, eval=FALSE}
